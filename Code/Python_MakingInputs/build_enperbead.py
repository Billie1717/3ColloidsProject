...
def main():
    ...
    rc_c = 1.5 #(sigma_c+2)/(sigma_c+1) #ensures rcut ends up being the radius of the colloid plus 1 membrane bead
    kappa = 15
    press  = 1.0
    D0 = float(sys.argv[5])
    seed = int(sys.argv[6])
...
\n'''%(outfile, outfile))    #cd $TMPDIR
    #fsub.write("/nfs/scistore15/saricgrp/bmeadowc/Scratch/Teather/MC_mem_clean/scripts/Vescicle_TotEnCalc2 in.ves\n") 
    fsub.write("/nfs/scistore15/saricgrp/bmeadowc/Scratch/Teather/MC_mem_clean/scripts/Vescicle_EPB_averaged in.ves\n") #change to lammps dr
