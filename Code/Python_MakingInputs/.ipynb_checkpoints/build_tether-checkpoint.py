...
def positions(arc1,arc2,number, radius, tau):
    x = []
    y = []
    z = []
...
def main():
    ...
    tau = int(sys.argv[3])
    sigma_c = float(sys.argv[4])
    rc_c = 1.2 #ALL COLLOID PAPER DATA WITH RC_C = 1.5 #(sigma_c+2)/(sigma_c+1) #ensures rcut ends up being the radius of the colloid plus 1 membrane bead
    kappa = 15
    press  = 1.0
    D0 = float(sys.argv[5])
    seed = int(sys.argv[6])
    
    print('intput params', arc1,arc2,tau,D0)
    
    #filepattern = sys.argv[1] #'colloids_arc1_%d_arc2_%d_tau_%d_D0_%d'%(arc1,arc2,tau,D0)
    #print('filepattern',filepattern)
    workingdir = '/Users/billiemeadowcroft/Dropbox/PhDGithub/RemoteTether/Code/' ## YOUR DIRECTORY HERE
    fname_in = workingdir +'Vesicle_file/icos_%d_0_8.dat'%(Nmem) 			# read in positions of membrane particles
    ...
    ################ Cargo particles ################
    Ndumb = 3
    radius1 = cell_radius + sigma_c
    radius2 = 22.695 +sigma_c*0.2 #cell_radius + sigma_c*0.5 23.195
    x_bead, y_bead, z_bead = positions(arc1,arc2, Ndumb, radius1,tau)
    x_beadSp, y_beadSp, z_beadSp = positions(arc1,arc2, Ndumb, radius2,tau)
    Ntot = Nmem + Ndumb
...
\n'''%(outfile, outfile))    #cd $TMPDIR
    fsub.write("/nfs/scistore15/saricgrp/bmeadowc/Scratch/Teather/MC_mem_clean/scripts/Vescicle_Tether in.ves\n") 
