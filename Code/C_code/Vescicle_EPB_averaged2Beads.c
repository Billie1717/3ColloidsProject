...
typedef struct{
  ...
  double bindAtom, stretchAtom, bendAtom, numAtom;
  ...
  set_all();
   ...
   for (i=1;i<=MCSTEPS;i++){ //This is where the loop is
    ...
    if (i == 1 || i%WRITE_CONF==1 && i>10) {write();painter3();painter();painter_en(i,counter, N, Ncoll,Ntri);counter+=1;
            for (j=1;j<=N;j++){Elem[j].bindAtom = 0;Elem[j].bendAtom = 0;Elem[j].stretchAtom = 0;Elem[j].numAtom = 0;}}
  }
...
void set_Elements(void){
  ...
  for (t=1;t<=N+Ncoll;t++){ 
    ...
    Elem[t].bindAtom = 0;
    Elem[t].bendAtom = 0;
    Elem[t].stretchAtom = 0;
    Elem[t].numAtom = 0;
  }  
    
  theta1 = atan((sqrt(Elem[N+Ncoll-1].x*Elem[N+Ncoll-1].x+Elem[N+Ncoll-1].y*Elem[N+Ncoll-1].y)/Elem[N+Ncoll-1].z));
  theta2 = atan((sqrt(Elem[N+Ncoll].x*Elem[N+Ncoll].x+Elem[N+Ncoll].y*Elem[N+Ncoll].y)/Elem[N+Ncoll].z));
  ...
  phi1 = atan(Elem[N+Ncoll-1].y/Elem[N+Ncoll-1].x);
  phi2 = atan(Elem[N+Ncoll].y/Elem[N+Ncoll].x);

  printf("theta1 2, phi1 2: %.2f %.2f %.2f %.2f\n",theta1,theta2,phi1, phi2);    
 ...
 //coll-bead verlet list: 
  ...
  bind_n=bind_1p(k);
   ...
   if (bind_n==999){  // reject
    ...
    S.Svol=DV2;
    Elem[k].bendAtom += en;
    Elem[k].bindAtom += bind_n;
    Elem[k].stretchAtom += S.pres*DV2;
    Elem[k].numAtom += 1;
...
void painter(void){
 ...
 for (p=1;p<=N+Ncoll;p++){ //printf("p=%d N=%d\n",p,Elem[p].Ng);
   ...
   if (p<=N) fprintf(o,"%d %f %f %f %f %f %f\n",
			     Elem[p].type,Elem[p].x,Elem[p].y,Elem[p].z ,Elem[p].bindAtom/Elem[p].numAtom ,Elem[p].bendAtom/Elem[p].numAtom,Elem[p].stretchAtom/Elem[p].numAtom);
    else fprintf(o,"%d %f %f %f %f %f %f\n", Elem[p].type,Elem[p].x,Elem[p].y,Elem[p].z ,Elem[p].bindAtom/Elem[p].numAtom ,Elem[p].bendAtom/Elem[p].numAtom ,Elem[p].stretchAtom/Elem[p].numAtom);
...
thetacoll = atan((sqrt((oldx-Centre.x)*(oldx-Centre.x)+(oldy-Centre.y)*(oldy-Centre.y))/(oldz-Centre.z)));
phicoll = atan((oldy-Centre.y)/(oldx-Centre.x));
...
thetanew = atan((sqrt((Elem[coll].x-Centre.x)*(Elem[coll].x-Centre.x)+(Elem[coll].y-Centre.y)*(Elem[coll].y-Centre.y))/(Elem[coll].z-Centre.z)));
phinew = atan((Elem[coll].y-Centre.y)/(Elem[coll].x-Centre.x));
